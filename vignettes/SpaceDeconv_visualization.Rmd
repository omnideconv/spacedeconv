...
spatial_data_1 <- SpaceDeconv::normalize(spatial_data_1, method = "cpm")
...
SpaceDeconv::plot_celltype(deconv, cell_type = "quantiseq_B.cell")
```
...
SpaceDeconv::plot_cells_per_spot(deconv, threshold = 0.05)
```
...
SpaceDeconv::plot_umi_count(deconv, transform_scale = "sqrt")
```
...
SpaceDeconv::plot_most_abundant(deconv,
  method = "quantiseq",
  palette = "Viridis",
  remove = "quantiseq_uncharacterized.cell"
)
```
...
* `discrete` logical, whether to scale the color with discrete values
