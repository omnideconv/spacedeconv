    ...
    for (i in 1:length(rownames(mat_coloc))) {
      for (j in 1:length(colnames(mat_coloc))) {
        ...
        ht1 <- Heatmap(mat_coloc,
                       ...
                       cell_fun = function(j, i, x, y, w , h , fill) {
                         if (i > j) {
                           ...
                           grid.text(mat_coloc[i,j], x , y, gp = gpar(fontsize = 10))
                         }
                       }
        )
        ...
        ht2 <- Heatmap(mat_avoid,
                       heatmap_legend_param = list(
                         title = "Avoidance", at = c(0, alpha-2*alpha/3, alpha-alpha/3 ,alpha, 1),
                         break_dist = 1,
                         labels = c("***", "**", "*", alpha, "ns")),
                       ...
                       cell_fun = function(j, i, x, y, w, h, fill) {
                         if (i > j) {
                           ...
                           grid.text(mat_avoid[i,j], x ,y, gp = gpar(fontsize = 10))
                         }
                       }
        )
      }
    }
    draw(ht1, ht_gap = unit(-75, "mm"))
    draw(ht2, ht_gap = unit(-75, "mm"))
    if (matrix == TRUE) {
      print("mat_coloc")
      print(mat_coloc)
      print("mat_avoid")
