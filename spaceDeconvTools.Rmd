...
```{r Datasets, echo=TRUE, error=FALSE, message=FALSE, warning=FALSE, results='hide'}
# Spatial
...
groups = colData(sc)[[slot]]
# norm 
sc <- scuttle::logNormCounts(sc) # score marksers

mgs <- scran::scoreMarkers(sc) # can include a subset step, requires logcounts assay!!!
...
model <- trainNMF(sc, sp, mgs=mgs_df, weight_id = "mean.AUC", groups = groups)



deconvolution <- SPOTlight::SPOTlight(sc, sp, mgs=mgs_df, weight_id = "mean.AUC", verbose=TRUE, groups = groups)
