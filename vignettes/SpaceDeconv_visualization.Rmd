...
SpaceDeconv::plot_celltype(deconv, cell_type = "rctd_Cancer.Epithelial", palette = "Mako", spot_size = 1.17)
```
...
SpaceDeconv::plot_cells_per_spot(deconv, threshold = 0.01, palette = "Mako", spot_size = 1.17)
```

For the UMI count spot no additional information is necessary. Further no deconvolution result is required.

```{r plotUMIcount, fig.height=7, fig.width=7}
SpaceDeconv::plot_umi_count(deconv, palette = "Mako", spot_size = 1.17)
```
...
plot_cells_per_spot(deconv, offset_rotation = TRUE) # rotate hexagons
```
### Transform Scale 
...
```{r}
SpaceDeconv::plot_celltype(deconv, cell_type = "rctd_Cancer.Epithelial", palette = "Mako", spot_size = 1.17, smooth = T, smoothing_factor = 1.5)
```
