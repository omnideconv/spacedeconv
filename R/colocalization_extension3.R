...
localization_heatmap <- function(spe, method, distance = 0, correlation = TRUE, localization = TRUE, matrix = FALSE, presence_matrix = NULL) {
  ...
  if (localization == TRUE) {
    ...
    corrplot(ratio_coloc,
      p.mat = p_coloc, method = "color", diag = FALSE, type = "lower",
      is.corr = FALSE,
      col = COL1("Blues"),
    ...
    corrplot(ratio_avoid,
      p.mat = p_avoid, method = "color", diag = FALSE, type = "lower",
      is.corr = FALSE,
      col = COL1("Reds"),
      sig.level = c(0.001, 0.01, 0.05), pch.cex = 0.9,
      insig = "label_sig", pch.col = "black", tl.col = "black",
  ...
  if (correlation == TRUE) {
    ...
    corrplot(correlation,
      p.mat = cor_prob, method = "color", diag = FALSE, type = "lower",
      sig.level = c(0.001, 0.01, 0.05), pch.cex = 0.9,
      insig = "label_sig", pch.col = "black", tl.col = "black",
